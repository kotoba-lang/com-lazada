(ns lazada.main
  "Kotodama WASM entrypoint for the Lazada clean-room actor (L4) — Clojure port.
  ...
  facts (`lazada.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lazada")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"lazada.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lazada-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lazada-compat/ready)
